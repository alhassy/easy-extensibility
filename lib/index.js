 ...
 * ## Example Usage
 * ```
 * // For local testing, pass in a stub implementation of `vscode`
 * let vscode = {window: {showInformationMessage: console.error}}
 * let E = require('./index.js')(vscode)
 * ```
 * The default value for `vscode` is a "just enough" stub so that you can
 * load the package without any arguments provided.
 *
 * ```
 * // More realistically, in your extension.js you would have:
...
module.exports = (vscode = {
  window: {
    createTextEditorDecorationType: x => x,
    onDidChangeTextEditorSelection: x => x
  },
  env: { clipboard: {} },
  commands: { executeCommand: x => x }
}) => {
  const E = {}
  ...
  */
   ...
   * ### Example Use
   * ```
   * E.shell("fortune").then(x => E.webPanel('Fortune!', `<marquee>${x.stdout}</marquee>`))
   * ```
   *
  ...
  E.internal.evaluateSelection = async currentPrefixArgument => {
    ...
    E.internal.require = { NODE_PATH: (await E.shell('npm root -g')).stdout.trim() }

    let now = (await E.shell("date +%H:%M:%S")).stdout.trim()
  ...
  E.internal.executeRegisteredCommand = async currentPrefixArgument => {
